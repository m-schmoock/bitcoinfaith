...
Source10:	https://raw.githubusercontent.com/bitcoinfaith/bitcoinfaith/master/contrib/debian/examples/bitcoinfaith.conf
...
cp -p %{SOURCE10} ./bitcoinfaith.conf.example
tar -zxf %{SOURCE1}
...
cat <<EOF > %{buildroot}%{_tmpfilesdir}/bitcoinfaith.conf
d /run/bitcoind 0750 bitcoin bitcoin -
EOF
touch -a -m -t 201504280000 %{buildroot}%{_tmpfilesdir}/bitcoinfaith.conf
...
CONFIG_FILE="%{_sysconfdir}/bitcoin/bitcoinfaith.conf"
DATA_DIR="%{_localstatedir}/lib/bitcoin"
PID_FILE="/run/bitcoind/btfd.pid"
EOF
...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 8345
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 8333
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 18345
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 18346
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 8345
	%{_sbindir}/semanage port -d -p tcp 8333
	%{_sbindir}/semanage port -d -p tcp 18345
	%{_sbindir}/semanage port -d -p tcp 18346
	for selinuxvariant in %{selinux_variants}; do
...
%doc COPYING bitcoinfaith.conf.example doc/README.md doc/bips.md doc/files.md doc/multiwallet-qt.md doc/reduce-traffic.md doc/release-notes.md doc/tor.md
...
%defattr(-,root,root,-)
%license COPYING db-%{bdbv}.NC-LICENSE
%doc COPYING bitcoinfaith.conf.example doc/README.md doc/REST-interface.md doc/bips.md doc/dnsseed-policy.md doc/files.md doc/reduce-traffic.md doc/release-notes.md doc/tor.md
%attr(0755,root,root) %{_sbindir}/bitcoind
%attr(0644,root,root) %{_tmpfilesdir}/bitcoinfaith.conf
...
%defattr(-,root,root,-)
%license COPYING
%doc COPYING bitcoinfaith.conf.example doc/README.md
