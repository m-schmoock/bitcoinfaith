...
UniValue getpeerinfo(const JSONRPCRequest& request)
{
    ...
    for (const CNodeStats& stats : vstats) {
        ...
        obj.push_back(Pair("btfmagic", stats.fUsesGoldMagic));
...
UniValue addnode(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2 ||
        ...
        throw std::runtime_error(
            ...
            "\nExamples:\n"
            + HelpExampleCli("addnode", "\"192.168.0.6:8346\" \"onetry\"")
            + HelpExampleRpc("addnode", "\"192.168.0.6:8346\", \"onetry\"")
        );
...
UniValue disconnectnode(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() == 0 || request.params.size() >= 3)
        throw std::runtime_error(
            ...
            "\nExamples:\n"
            + HelpExampleCli("disconnectnode", "\"192.168.0.6:8346\"")
            + HelpExampleCli("disconnectnode", "\"\" 1")
            + HelpExampleRpc("disconnectnode", "\"192.168.0.6:8346\"")
...
UniValue getaddednodeinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "       {\n"
            "         \"address\" : \"192.168.0.201:8346\",  (string) The bitcoin server IP and port we're connected to\n"
